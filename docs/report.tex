...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.85\linewidth]{sq4.png}
...
\begin{figure}[H]
  ...
  \caption{AS-02 -- Link Patient Identity to Health Patterns: \textbf{Residual Risk}}
\end{figure}
...
\begin{figure}[H]
  ...
  \caption{AS-04 -- Modify or Reassign Appointment Records: \textbf{Prevented}}
\end{figure}
...
\begin{figure}[H]
  ...
  \caption{AS-05 -- Destroy Patient Records via Cascade Deletion: \textbf{Prevented}}
\end{figure}
...
\begin{figure}[H]
  ...
  \caption{AS-06 -- Perform Undetectable Actions: \textbf{Residual Risk}}
\end{figure}
...
\begin{figure}[H]
  ...
  \caption{AS-07 -- Overload API / Denial of Service: \textbf{Prevented}}
\end{figure}
...
\textbf{Control} & \textbf{Implementation detail} \\
\midrule
Rate limiting          & \texttt{RateLimitFilter} -- 100 req/min per IP enforced at application layer \\
HTTP 429 + Retry-After & Clients receive proper backpressure signal with \texttt{Retry-After: 60} header \\
POST flood blocked     & \texttt{POST /api/appointments} also requires JWT + RECEPTIONIST/ADMIN role -- anonymous flood returns 401 \\
Request size limits    & \texttt{server.tomcat.max-http-form-post-size=1MB} -- oversized payloads rejected \\
Perimeter rate limiting & Nginx \texttt{limit\_req\_zone} enforces 30 req/s burst / 10 req/s sustained per IP at the WAF perimeter \\
\bottomrule
\end{tabularx}
\end{table}
...
The nightly scan flagged a missing \texttt{X-Content-Type-Options} header. The PR scan
additionally flagged a missing \texttt{Strict-Transport-Security} header and a Unix
timestamp disclosure.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.75\linewidth]{dast-part1-nightly.png}
  \caption{Part 1 nightly ZAP report: 0 High, 0 Medium, 1 Low}
\end{figure}

\subsection{Part 2 Results}

In Part 2 the security header set was greatly expanded. This paradoxically increased the
number of ZAP findings because ZAP now has more headers to evaluate and flags stricter
requirements such as COEP and COOP headers that were not expected on the baseline.

\begin{table}[H]
\centering
\caption{Part 2 DAST findings (direct scan vs WAF scan)}
\begin{tabular}{lllll}
\toprule
\textbf{Run} & \textbf{Scan mode} & \textbf{High} & \textbf{Medium} & \textbf{Low} \\
\midrule
Nightly & Direct (no WAF) & 0 & 3 & 2 \\
Nightly & Via WAF         & 0 & 3 & 2 \\
PR      & Direct (no WAF) & 0 & 3 & 2 \\
PR      & Via WAF         & 0 & 3 & 2 \\
\bottomrule
\end{tabular}
\end{table}

All four runs return identical findings, which confirms that the WAF does not introduce
new issues and that the same CSP gaps exist regardless of whether traffic passes through
ModSecurity. The three Medium findings are all CSP-related: a missing fallback directive,
\texttt{script-src unsafe-inline} and \texttt{style-src unsafe-inline}. These are accepted
trade-offs for the current React frontend which relies on inline scripts and styles during
the build. The two Low findings are missing \texttt{Cross-Origin-Embedder-Policy} and
\texttt{Cross-Origin-Opener-Policy} headers, which are optional isolation headers not
required by the project scope.

No High severity findings were present in any Part 2 scan. The findings that existed in
Part 1 (missing HSTS, missing X-Content-Type-Options) are no longer present because both
headers are now set explicitly in the Nginx configuration.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.65\linewidth]{dast-part2-nightly-direct.png}
  \caption{Part 2 nightly ZAP report (direct scan): 0 High, 3 Medium, 2 Low}
\end{figure}

\begin{figure}[H]
  \centering
  \includegraphics[width=0.65\linewidth]{dast-part2-nightly-waf.png}
  \caption{Part 2 nightly ZAP report (via WAF): 0 High, 3 Medium, 2 Low}
\end{figure}
...
(AS-07). No abuse story remains at Residual Risk. UUID primary keys were implemented, eliminating
