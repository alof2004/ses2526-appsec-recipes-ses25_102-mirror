...
Both workflows use \textbf{OWASP ZAP} automation plans. The application is launched inside Docker with a temporary PostgreSQL container, a backend container exposing the Spring Boot API, and a frontend container exposing the React/Vite application. The PR workflow scans the direct application stack. The nightly workflow scans both the direct stack and the WAF-fronted stack.
