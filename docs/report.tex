...
D-01 & No rate limiting & \textbf{\textcolor{sevlow}{Mitigated}} &
  Dual-layer: Nginx \texttt{limit\_req\_zone} (30 req/s per IP) at the perimeter;
  Spring \texttt{RateLimitFilter} (100 req/min per IP) at the application layer. \\[6pt]
...
AppointmentRequestDTO       & Accepts only \texttt{dateTime}, \texttt{specialty}, \texttt{status} -- no patient field in body \\
Mass-assignment blocked     & \texttt{spring.jackson.deserialization.fail-on-unknown-properties=true} rejects extra fields \\
Patient rebinding prevented & DTO has no patient field -- the service \texttt{update()} patient branch is structurally unreachable \\
Input validation            & \texttt{@NotBlank}, \texttt{@NotNull} on all DTO fields -- invalid payloads return 400 with field errors \\
\bottomrule
\end{tabularx}
\end{table}
...
DOCTOR / RECEPTIONIST blocked & Non-admin authenticated users receive 403 Forbidden on DELETE -- role matrix enforced \\
Rate limiting                 & \texttt{RateLimitFilter} -- 100 req/min per IP; bulk delete loops are throttled even for ADMIN \\
\bottomrule
\end{tabularx}
\end{table}
...
\textbf{Control} & \textbf{Implementation detail} \\
\midrule
JWT authentication           & Every \texttt{/api/**} request now carries a verified Keycloak identity (\texttt{sub}, \texttt{preferred\_username}, roles) \\
Identity extraction          & \texttt{JwtRoleConverter} maps \texttt{realm\_access.roles} to \texttt{ROLE\_*} authorities; \texttt{sub} claim identifies the actor \\
Spring Security filter chain & All requests pass through the security layer -- the prerequisite for attribution is now in place \\
Unauthenticated actions blocked & Anonymous requests return 401 -- the anonymous attacker profile from C1 is eliminated \\
\midrule
\multicolumn{2}{p{\linewidth}}{\textit{Residual risk: no structured audit log has been implemented. Authenticated actions are attributable to a JWT subject, but no interceptor writes a change record to a persistent store. Full resolution requires a structured access/audit log (Spring AOP interceptor or database trigger).}} \\
\bottomrule
\end{tabularx}
\end{table}
