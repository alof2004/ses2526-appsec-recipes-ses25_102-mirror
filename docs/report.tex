...
In Part 2 the DAST pipeline was upgraded from a passive baseline scan to a full active scan
using \texttt{automation-plan.yaml}, targeting both frontend and backend endpoints with
SQLi, XSS, path traversal and SSTI rules at HIGH strength. Each run was performed twice:
once directly against the application and once through the WAF, so the contribution of
ModSecurity CRS can be isolated.

\begin{table}[H]
\centering
\caption{Part 2 DAST findings -- active scan, direct vs WAF}
...
Nightly & Via WAF         & 0 & 0 & 2 & Server version leak, in-page banner info leak \\
PR      & Direct (no WAF) & 0 & 3 & 2 & CSP: unsafe-inline; missing COEP/COOP headers \\
PR      & Via WAF         & 0 & 3 & 2 & CSP: unsafe-inline; missing COEP/COOP headers \\
\bottomrule
\end{tabularx}
\end{table}
...
is not a directive with a \texttt{default-src} fallback. The two Low findings in the PR scan
(\texttt{Cross-Origin-Embedder-Policy} and \texttt{Cross-Origin-Opener-Policy}) are optional
isolation headers outside the project scope.

The nightly WAF scan shows a different finding profile to the direct scan: the WAF introduces
two Low findings from Nginx leaking its version string in the \texttt{Server} header and in
page banners. These are resolved by adding \texttt{server\_tokens off} to the WAF Nginx
configuration. The PR scans are identical direct vs WAF, confirming ModSecurity CRS does not
suppress or alter the CSP-related findings.
...
\begin{figure}[H]
  ...
  \caption{Part 2 nightly ZAP active scan (via WAF): 0 High, 0 Medium, 2 Low}
\end{figure}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.75\linewidth]{dast-p2-pr-waf.png}
  \caption{Part 2 PR ZAP scan (via WAF): 0 High, 3 Medium, 2 Low}
\end{figure}
