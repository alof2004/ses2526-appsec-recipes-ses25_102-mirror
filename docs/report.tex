...
libc, zlib and the same npm packages. The Part 2 scan therefore shows more raw findings than
Part 1, which reflects the passage of time rather than a regression in the codebase.

To address the OS package CVEs, both Dockerfiles were updated to run a full package upgrade
at build time: \texttt{apk upgrade --no-cache} in the frontend and \texttt{apt-get upgrade -y}
in the backend. This ensures the images always pull the latest patched versions when built.

\begin{table}[H]
\centering
\caption{Part 2 Trivy image scan findings before Dockerfile hardening}
\begin{tabular}{lll}
\toprule
\textbf{Image} & \textbf{Findings} & \textbf{Category} \\
\midrule
Frontend & 40 (1 CRITICAL, 39 HIGH) & libssl3, musl, zlib (OS); npm build-time packages \\
Backend  & 11 (11 HIGH)             & OpenSSL (OS); Go stdlib in JDK tooling \\
\bottomrule
\end{tabular}
\end{table}

The npm CVEs are confined to the Node.js build stage and are not present in the final Nginx
image, so they carry no runtime risk. The Go stdlib CVEs in the backend are embedded in
binaries that ship inside the JRE image itself and cannot be resolved by an OS upgrade;
they are accepted as residual risk in third-party tooling. The nightly image scan after
the Dockerfile hardening is pending the merge of the \texttt{develop} branch to
\texttt{master}.
