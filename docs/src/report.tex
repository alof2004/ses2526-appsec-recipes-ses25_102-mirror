...
I-06 & Backend port directly accessible & \textbf{\textcolor{sevlow}{Mitigated}} &
  ...
  through the WAF (\texttt{waf:8080}). PostgreSQL likewise has no host-published port;
  the database is reachable only on the internal Docker network by the backend service. \\[6pt]

D-01 & No rate limiting & \textbf{\textcolor{sevlow}{Mitigated}} &
  Dual-layer: Nginx \texttt{limit\_req\_zone} (30 req/s sustained, burst 60 nodelay per IP)
...
\textbf{Record-level:} three role-specific filters are enforced server-side in the service layer:

\begin{itemize}
  \item \textbf{ADMIN} -- unrestricted access to all records; every mutating operation (CREATE, UPDATE, DELETE) is written to the \texttt{audit\_logs} table with the actor's JWT \texttt{sub}, resource type, resource ID, and timestamp.
  \item \textbf{DOCTOR} -- the \texttt{Appointment} entity carries a \texttt{doctorSub} field (the JWT \texttt{sub} of the assigned doctor). The filter \texttt{doctorSub == JWT sub} is applied server-side, never client-side; the caller cannot bypass it by omitting or spoofing the field. Only appointments matching the caller's own \texttt{sub} are returned or accepted for update, and patients are restricted to those linked via at least one such appointment. Accessing a record not assigned to the caller returns HTTP 404, indistinguishable from a non-existent resource.
  \item \textbf{RECEPTIONIST} -- read access to all records. Write and delete operations on appointments are restricted to records where \texttt{createdBy} equals the caller's JWT \texttt{sub}, preventing modification of appointments created by other staff.
\end{itemize}
...
is enforced server-side in the service layer per role: \texttt{DOCTOR} -- \texttt{doctorSub == JWT sub} filter is applied server-forced (not client-controlled), returning 404 for unowned records; \texttt{RECEPTIONIST} -- write/delete on appointments restricted to \texttt{createdBy == JWT sub}; \texttt{ADMIN} -- unrestricted access with full audit trail. \\[6pt]
...
\texttt{RateLimitFilter} (token-bucket, 100\,req/min per IP) on all \texttt{/api/**} routes. Request-size limits set to 1\,MB via Tomcat (\texttt{max-http-form-post-size}) and Spring multipart (\texttt{max-request-size}, \texttt{max-file-size}). \\[6pt]
...
\texttt{SecurityConfig} configures HSTS, \texttt{X-Frame-Options: DENY}, \texttt{X-Content-Type-Options}, and \texttt{Content-Security-Policy: default-src 'none'}. Form login and HTTP Basic are disabled. Secure cookie flags (\texttt{Secure}, \texttt{HttpOnly}, \texttt{SameSite}) are not applicable: the API is fully stateless and uses Bearer tokens only -- no session or authentication cookies are issued at any point. \\[6pt]
...
IP rate limiting    & Nginx \texttt{limit\_req\_zone} -- 30 req/s sustained, burst 60 nodelay per IP \\
...
\begin{figure}[H]
  \centering
  \makebox[\textwidth][c]{\includegraphics[width=1.25\textwidth]{arch.png}}
  \caption{Zero Trust Architecture -- component and data-flow overview}
  \label{fig:zta-arch}
\end{figure}
...
AS-08 & Overload the API with Repeated Requests &
  ...
  enforces 30 req/s sustained, burst 60 nodelay per IP at the perimeter; (2) Spring
...
The following figures present the regression diagrams for each abuse story, showing the original attack path and the resulting status. Each diagram is paired with the table of controls applied in Part 2.

\newcommand{\regressionImage}[2]{%
  \vspace{0.5em}
  \includegraphics[width=0.95\linewidth,height=0.42\textheight,keepaspectratio]{#1}
  \captionof{figure}{#2}
}
\newcommand{\regressionImageTall}[2]{%
  \vspace{0.5em}
  \includegraphics[width=0.95\linewidth,height=0.54\textheight,keepaspectratio]{#1}
  \captionof{figure}{#2}
}
...
\captionof{table}{AS-01 -- Controls applied in Part 2}
...
\bottomrule
\end{tabularx}
\regressionImage{01_regression.png}{AS-01 -- Unauthorised Access to Patient Data: \textbf{Prevented}}
\end{minipage}
\par\medskip
...
\scriptsize
\renewcommand{\arraystretch}{0.88}
\captionof{table}{AS-02 -- Controls applied in Part 2}
\begin{tabularx}{\linewidth}{>{\bfseries}p{4.2cm} X}
\toprule
\textbf{Control} & \textbf{Implementation detail} \\
\midrule
...
\bottomrule
\end{tabularx}
\regressionImage{02_regression.png}{AS-02 -- Link Patient Identity to Appointment History: \textbf{Prevented}}
\end{minipage}
\par\medskip
...
\scriptsize
\renewcommand{\arraystretch}{0.88}
\captionof{table}{AS-03 -- Controls applied in Part 2}
\begin{tabularx}{\linewidth}{>{\bfseries}p{4.2cm} X}
\toprule
\textbf{Control} & \textbf{Implementation detail} \\
\midrule
...
\bottomrule
\end{tabularx}
\regressionImage{03_regression.png}{AS-03 -- Direct Patient Identification via IDOR: \textbf{Prevented}}
\end{minipage}
\par\medskip
...
\scriptsize
\renewcommand{\arraystretch}{0.88}
\captionof{table}{AS-04 -- Controls applied in Part 2}
...
\bottomrule
\end{tabularx}
\regressionImage{04_regression.png}{AS-04 -- Infer Health Condition from Appointment Metadata: \textbf{Prevented}}
\end{minipage}
\par\medskip
...
\scriptsize
\renewcommand{\arraystretch}{0.88}
\captionof{table}{AS-05 -- Controls applied in Part 2}
\begin{tabularx}{\linewidth}{>{\bfseries}p{4.6cm} X}
\toprule
\textbf{Control} & \textbf{Implementation detail} \\
\midrule
...
Mass-assignment blocked     & \path{spring.jackson.deserialization.fail-on-unknown-properties=true} rejects extra fields \\
...
\bottomrule
\end{tabularx}
\regressionImage{05_regression.png}{AS-05 -- Modify or Reassign Appointment Records: \textbf{Prevented}}
\end{minipage}
\par\medskip
...
\scriptsize
\renewcommand{\arraystretch}{0.88}
\captionof{table}{AS-06 -- Controls applied in Part 2}
\begin{tabularx}{\linewidth}{>{\bfseries}p{4.2cm} X}
\toprule
\textbf{Control} & \textbf{Implementation detail} \\
\midrule
...
\bottomrule
\end{tabularx}
\regressionImage{06_regression.png}{AS-06 -- Destroy Patient Records via Cascade Deletion: \textbf{Prevented}}
\end{minipage}
\par\medskip
...
\scriptsize
\renewcommand{\arraystretch}{0.88}
\captionof{table}{AS-07 -- Controls applied in Part 2}
\begin{tabularx}{\linewidth}{>{\bfseries}p{4.2cm} X}
\toprule
\textbf{Control} & \textbf{Implementation detail} \\
\midrule
...
\bottomrule
\end{tabularx}
\regressionImage{07_regression.png}{AS-07 -- Perform Undetectable Actions: \textbf{Prevented}}
\end{minipage}
\par\medskip
...
\scriptsize
\renewcommand{\arraystretch}{0.88}
\captionof{table}{AS-08 -- Controls applied in Part 2}
\begin{tabularx}{\linewidth}{>{\bfseries}p{4.2cm} X}
\toprule
\textbf{Control} & \textbf{Implementation detail} \\
\midrule
...
Perimeter rate limiting & Nginx \texttt{limit\_req\_zone} enforces 30 req/s sustained, burst 60 nodelay per IP at the WAF perimeter \\
ModSecurity CRS         & Request-body limits (1\,MB) enforced at perimeter; malformed request floods blocked by CRS rules \\
\bottomrule
\end{tabularx}
\regressionImageTall{08_regression.png}{AS-08 -- Overload the API with Repeated Requests: \textbf{Prevented}}
\end{minipage}
\par\medskip
